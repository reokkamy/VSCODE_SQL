...
--기본문법 IN: 복수의 값중 하나와 일치하는 경우
--DALLAS에 위치한 부서의 부서번호중
--하나에 속한 사원의 이름을 출력예시
...
--ANY
--부서번호 30번에 속한 사원들 중 적어도 한 명 보다
--급여가 적은 사원의 이름을 출력예시
--부서 30번의 급여중 하나라도 큰값이 있다면 해당사원보다 적은 급여를 가진 사원을 모두 출력
...
-- ALL
--부서번호 30번의 모든 사원보다 급여가 많은 사원의 이름 출력
--ALL은 전부 만족해야 참이 되는조건
--가장 큰값보다 더 커야함
--MAX()와 비슷한 조건으로 사용
SELECT ENAME FROM EMP
WHERE SAL>ALL(
...
--EXISTS 연산자
--사원이 소속하는 부서의 이름을 출력
--하나라도 존재하면 참(TRUE) -> 매우 빠른조건에 존재확인에 유리(데이터 확인 아님)
--서브쿼리의 실제 데이터가 아니라 존재유무만 확인함
--반환값이 있으면 무조건 참
...
--30번 부서중 가장높은 급여보다 적게 급여를 받는 직원
SELECT ENAME, SAL FROM EMP
WHERE SAL< ALL(
SELECT SAL FROM EMP WHERE DEPTNO =30);
