--문자열 관련 내장 함수, 오라클에서 미리 만든기능
--우리는 이용하는 대부분을 공부함

-- 이름을 모두 대문자, 소문자, 첫글자 대문자등 출력해보기
SELECT ENAME, 
UPPER(ENAME) AS "사원명 대문자",
LOWER(ENAME) AS "사원명 소문자", 
INITCAP(ENAME) AS "첫글자 대문자" FROM EMP;

--사원이름의 길이 , 바이트 수 출력
SELECT ENAME, LENGTH(ENAME) AS "사원명 글자길이",
LENGTHB(ENAME) AS "사원명 바이트 길이" FROM EMP;
...
--문자열 바꾸기
REPLACE(JOB,'CLERK', '직원') AS "한글 직무" FROM EMP;

--LPAD, RPAD :포맷 맞추기
--LPAD(컬럼명(문자열), 총길이, '채워질문자')
--사원명에서 전체길이 9자리만들고,남은 부분은 채워질 문자로 채움
SELECT ENAME, LPAD(ENAME, 9, '*') AS "9글자포맷마스킹처리" FROM EMP;
